const titreDemarcheSansEtapes = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: []
}

const titreDemarcheOctAnf = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-anf01',
      typeId: 'anf',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titreDemarcheOctDpuAcc = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-dpu01',
      typeId: 'dpu',
      statutId: 'acc',
      ordre: 2
    },
    {
      id: 'h-cxx-courdemanges-1988-oct01-dex01',
      typeId: 'dex',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titreAxmDemarcheOctDexAcc = {
  id: 'h-axm-courdemanges-1988-oct01',
  titreId: 'h-axm-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-axm-courdemanges-1988-oct01-dex01',
      typeId: 'dex',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titreArmDemarcheOctDefAcc = {
  id: 'h-arm-courdemanges-1988-oct01',
  titreId: 'h-arm-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-arm-courdemanges-1988-oct01-def01',
      typeId: 'def',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titrePrxDemarcheOctRpuAcc = {
  id: 'h-prx-courdemanges-1988-oct01',
  titreId: 'h-prx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-prx-courdemanges-1988-oct01-rpu01',
      typeId: 'rpu',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titreDemarcheProDpuAcc = {
  id: 'h-cxx-courdemanges-1988-pro01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'pro',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-pro01-dpu01',
      typeId: 'dpu',
      statutId: 'acc',
      ordre: 2
    },
    {
      id: 'h-cxx-courdemanges-1988-pro01-dex01',
      typeId: 'dex',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titreDemarcheOctScoFai = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-sco01',
      typeId: 'sco',
      statutId: 'fai',
      ordre: 1
    }
  ]
}

const titreDemarcheOctDexAcc = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-dex01',
      typeId: 'dex',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titreDemarcheOctDexDpuAcc = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-dex01',
      typeId: 'dex',
      statutId: 'acc',
      ordre: 2
    },
    {
      id: 'h-cxx-courdemanges-1988-pro01-dpu01',
      typeId: 'dpu',
      statutId: 'acc',
      ordre: 1
    }
  ]
}

const titreDemarcheOctDexRej = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-dex01',
      typeId: 'dex',
      statutId: 'rej',
      ordre: 1
    }
  ]
}

const titreDemarcheOctMenIns = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-men01',
      typeId: 'men',
      date: '1988-03-11'
    }
  ]
}

const titreArmDemarcheOctMdpIns = {
  id: 'h-arm-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-mdp01',
      typeId: 'mdp',
      date: '1988-03-11'
    }
  ]
}

const titreArmDemarcheOctDefIns = {
  id: 'h-arm-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-def01',
      typeId: 'def'
    }
  ]
}

const titreDemarcheOctMenInd = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-men01',
      typeId: 'men',
      date: '4000-03-11'
    }
  ]
}

const titreDemarcheOctRet = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-ret01',
      typeId: 'ret'
    }
  ]
}

const titreDemarcheOctMdp = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-mdp01',
      typeId: 'mdp'
    }
  ]
}

const titreDemarcheOctMfr = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-mfr01',
      typeId: 'mfr'
    }
  ]
}

const titreDemarcheOctMcrDef = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-mcr01',
      typeId: 'mcr',
      statutId: 'def'
    }
  ]
}

const titreDemarcheOctMcrAcc = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-mcr01',
      typeId: 'mcr',
      statutId: 'acc',
      date: '1988-03-11'
    }
  ]
}

const titreDemarcheOctMcrInd = {
  id: 'h-cxx-courdemanges-1988-oct01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'oct',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-oct01-mcr01',
      typeId: 'mcr',
      statutId: 'acc',
      date: '4002-03-11'
    }
  ]
}

const titreDemarcheRetDpuFai = {
  id: 'h-cxx-courdemanges-1988-ret01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'ret',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-ret01-ret01',
      typeId: 'dpu',
      statutId: 'fai'
    }
  ]
}

const titreDemarcheRetDpuIni = {
  id: 'h-cxx-courdemanges-1988-ret01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'ret',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-ret01-ret01',
      typeId: 'dpu'
    }
  ]
}

const titreDemarcheRtd = {
  id: 'h-cxx-courdemanges-1988-ret01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'ret',
  statutId: 'acc',
  etapes: [
    {
      id: 'h-cxx-courdemanges-1988-ret01-rtd01',
      typeId: 'rtd'
    }
  ]
}

const titreDemarcheIndefinie = {
  id: 'h-cxx-courdemanges-1988-ret01',
  titreId: 'h-cxx-courdemanges-1988',
  typeId: 'ind',
  statutId: 'acc',
  etapes: [{ id: 'xxx', typeId: 'dpu', statutId: 'rej' }]
}

export {
  titreDemarcheSansEtapes,
  titreDemarcheOctAnf,
  titreDemarcheOctDpuAcc,
  titreAxmDemarcheOctDexAcc,
  titreArmDemarcheOctDefAcc,
  titrePrxDemarcheOctRpuAcc,
  titreDemarcheProDpuAcc,
  titreDemarcheOctScoFai,
  titreDemarcheOctDexAcc,
  titreDemarcheOctDexDpuAcc,
  titreDemarcheOctDexRej,
  titreDemarcheOctMenIns,
  titreArmDemarcheOctMdpIns,
  titreArmDemarcheOctDefIns,
  titreDemarcheOctMenInd,
  titreDemarcheOctRet,
  titreDemarcheOctMdp,
  titreDemarcheOctMfr,
  titreDemarcheOctMcrDef,
  titreDemarcheOctMcrAcc,
  titreDemarcheOctMcrInd,
  titreDemarcheRetDpuFai,
  titreDemarcheRetDpuIni,
  titreDemarcheRtd,
  titreDemarcheIndefinie
}
