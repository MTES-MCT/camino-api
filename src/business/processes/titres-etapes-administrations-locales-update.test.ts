import { ITitreAdministrationLocale, IAdministration } from '../../types'
  ...
  test("ajoute l'option associee à la Déal Guyane sur une ARM", async () => {
    ...
    expect(
      titresEtapesAdministrationsCreated.find(
        (titreAdministrationLocale: ITitreAdministrationLocale) =>
          titreAdministrationLocale.administrationId === 'dea-guyane-01'
      )!.associee
    ).toBeTruthy()
    expect(console.log).toHaveBeenCalled()
  })
